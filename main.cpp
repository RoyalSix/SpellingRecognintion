	...
	for (int i = 0; i < 26; i++)
	{
		...
		for (int j = 0; j < 26; j++)
		{
			cout << '\t'
				...
				<< prCharGivenCharOfState(asciiMap[j], asciiMap[i], scaleFactor) << endl;
			prSum += prCharGivenCharOfState(asciiMap[j], asciiMap[i], scaleFactor);
		}
...
void main()
{
	...
	while (inMenu)
	{
		...
		cout << "T. Recover an unknown message based on two data sets" << endl;
		cout << "B. Test typing the whole Biola vision article twice" << endl;
		cout << "A. Test automating spelling" << endl;
		cout << "R. Recover an unknown message based on one data set" << endl;
		cout << "D. Probability of typing document, given a document" << endl;
		cout << "L. Search for best spelling model parameters based on input text" << endl;
		cout << "U. Checking the precision of a recovered message." << endl;
		...
		case 'T': case 't'://
			cout << "Recover an unknown message based on two data sets:" << endl;
			testRecoverOriginal2();
			endOfService("Recover an unknown message based on two data sets");
			break;
		...
		case 'Z': case 'z'://
			...
			break;
		case 'D': case 'd'://
			cout << "[Probability of typing document, given a document]:" << endl;
			cout << "Pr(document 1 (person)| document 2 (biola))" << endl;
			testPrOfTypingDocument();
			endOfService("[Probability of typing document, given a document]");
			break;
		case 'L': case 'l'://
			cout << "Search for best spelling model parameters based on input text:" << endl;
			testgetPrFromDocument();
			endOfService("[Probability of typing document, given a document]");
			break;
		case 'U': case 'u'://
			cout << "Checking the precision of a recovered message." << endl;
			testPrecisionOfRecovered();
			endOfService("[Probability of typing document, given a document]");
			break;
